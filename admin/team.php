...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['image_file'])&&$_FILES['image_file']['error']===UPLOAD_ERR_OK) { $allowedMime=['image/jpeg','image/png','image/gif','image/webp']; $mime=mime_content_type($_FILES['image_file']['tmp_name']); $ext=strtolower(pathinfo($_FILES['image_file']['name'],PATHINFO_EXTENSION)); $allowedExt=['jpg','jpeg','png','gif','webp']; if(in_array($mime,$allowedMime)&&in_array($ext,$allowedExt)){$nm='team_'.time().'.'.$ext; if(!is_dir(__DIR__.'/../uploads'))mkdir(__DIR__.'/../uploads',0755,true); move_uploaded_file($_FILES['image_file']['tmp_name'],__DIR__.'/../uploads/'.$nm); $image='uploads/'.$nm;} }
